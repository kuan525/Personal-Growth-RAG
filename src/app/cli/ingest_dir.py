...
from src.app.ingestion.service import (
    ALLOWED_FILE_TYPES,
    DocumentIngestionError,
    DuplicateDocumentSkipped,
    ingest_local_file,
)
from src.app.storage.database import get_session_factory, init_db
    ...
    with session_factory() as db:
        for file_path in _iter_supported_files(directory):
            ...
            try:
                ...
                stats["created"] += 1
                logger.info(
                    "Created document: path=%s document_id=%s", file_path, response.document_id
                )
            except DuplicateDocumentSkipped as error:
                stats["skipped"] += 1
                logger.info(
                    "Skipped duplicate: path=%s document_id=%s", file_path, error.document_id
                )
